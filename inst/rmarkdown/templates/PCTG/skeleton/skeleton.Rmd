---
title: "Indifference template"
author: "Dr Conal Monaghan"
date: "`r format(Sys.time(), '%d %B %Y')`"
mail: "conal.monaghan@anu.edu.au"
linkedin: ""
twitter: ""
github: ""
home: ""
...
library(epuRate)      # devtools::install_github("ConalMonaghan/IndiffRate", force=TRUE)
library(bibtex)
```
...
# And our modified version of epuRate. If you don't already, install "devtools"
library(epuRate)      # devtools::install_github("ConalMonaghan/IndiffRate", force=TRUE)

# It can also be handy to download remedy which adds a dropdown menu for widely used RMD commands (only needs to be turned on once)
#library(remedy)       # remotes::install_github("ThinkR-open/remedy")
```
...
can also have hyperlinks to the applicable section in the important points through For example, [this link](#cache)
...
# Internal links {#link}
***
You can use internal links in R Markdown using anchors. For example, [this link](#cache) will bring you to the previous chapter. To do it:

- 1 - Put an 'anchor' next to a title. This is how the previous section title looks like:
```{r, eval=FALSE}
# You can link to the cached section {#cache}
```

- 2 - Put a link toward this anchor:
```{r, eval=FALSE}
For example, [this link](#cache) will bring ...
```
...
```{r bib, echo=FALSE, include= FALSE}
# Write a references file for all loaded packages
bibs <- write.bib(c((.packages())), file='references')
```
